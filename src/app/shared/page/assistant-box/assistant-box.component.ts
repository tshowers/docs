...
import { DocsAssistantCapabilitiesService } from '../../../services/docs-assistant-capabilities.service';
...
/**
 * Docs' own assistant-box - trimmed port of TODD's
 * shared/page/assistant-box/assistant-box.component.ts, same scoping
 * decision as web-products/network, web-products/pulse, and
 * web-products/moves' ports. Unlike those three, Docs keeps TWO domain LLM
 * services (Document + Knowledge) and the Knowledge-intents pre-check step,
 * since Docs genuinely owns both domains in TODD's original.
 */
@Component( {
...
export class AssistantBoxComponent implements OnInit, OnChanges, OnDestroy {
    ...
    assistantResponse = '';
    isLoading = false;
    @Input() placeholder = 'Ask about a document, or tell me what you need.';
    PRODUCTION = false;
    ...
    constructor (
        ...
        private capabilities: DocsAssistantCapabilitiesService,
    ...
    private buildContextSignature (): string | null {
        ...
        const signature = {
            ...
            summary: {
                totalDocumentCount: summary['totalDocumentCount'] ?? null,
            }
        };
    ...
    async askAssistant (): Promise<void> {
        ...
        const helpers: AssistantEngineHelpers = {
            ...
            runGlobalPreChecks: ( raw: string ) => {
                ...
                const routeIntentGuide = this.capabilities.tryRouteIntentGuide( raw );
                if ( routeIntentGuide.handled && routeIntentGuide.kind === 'message' ) {
                    patches.setAssistantResponse( this.boxHelper.normalizeAssistantHtml( routeIntentGuide.message ) );
                    patches.setPendingAction( null );
                    patches.setShowConfirmPrompt( false );
                    io.scheduleScrollToBottom( true );
                    return true;
            ...
            handleDocumentLLMIntent: ( raw: string ) => {
                this.openAISubscription?.unsubscribe?.();
                const promptWithContext = raw;
                const history = this.buildHistoryForLLM();
