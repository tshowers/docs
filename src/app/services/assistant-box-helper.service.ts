 ...
 * backtick-stripping term list are cut down to Docs' own routes only -
 * same trim as web-products/network, web-products/pulse, and
 * web-products/moves' ports.
 */
...
export class AssistantBoxHelperService {
  ...
  private routeLabelMap: Record<string, string> = {
    ...
    '/docs/editor': 'Editor',
    '/docs/upload': 'Upload',
    '/docs/proposal-history': 'Proposal History',
    '/docs/rfp-upload': 'RFP Upload',
    '/docs/rfp-list': 'RFPs',
    '/knowledge': 'Knowledge Base',
    '/knowledge/response-flow': 'Response Flow',
    '/help': 'Help',
  };
  ...
  public stripBackticksAroundRoutes ( s: string ): string {
    ...
    const terms = ['Knowledge', 'Documents', 'Editor', 'Upload', 'Knowledge Base', 'Response Flow', 'Home'];
  ...
  public tryWhatWorkIntent ( p: string ): string | null {
    if ( /\b(what work do you do|what do you actually do|do you actually do work|what work you do)\b/i.test( p ) ) {
      ...
      <ul>
        <li>Flag stale or duplicate documents before they cause confusion</li>
        <li>Turn documents into reusable knowledge entries</li>
        <li>Draft proposals from RFPs and past proposal history</li>
        <li>Help you find the right document or knowledge entry fast</li>
      </ul>
    `);
    }
    return null;
