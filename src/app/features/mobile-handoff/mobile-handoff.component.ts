 ...
 * web-products/outreach's own `mobile-handoff.component.ts` - same
...
export class MobileHandoffComponent implements OnInit {
  ...
  async ngOnInit (): Promise<void> {
    ...
    try {
      ...
      // the promise let the next route's component (e.g. DocumentListComponent,
      // which reads the signed-in user during its own init) mount before
      // that listener updated getUser(), so it initialized as if signed
      // out and rendered blank. Waiting for the real emission here closes
      // that race.
